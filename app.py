...
st.subheader("KM ChatBot for Verizon", divider="rainbow", anchor=False)

llm = ChatGroq(groq_api_key=groq_api_key,
               model_name='mixtral-8x7b-32768')

prompt_template = ChatPromptTemplate.from_template("""
Answer the following question based only on the provided context restrict to the provided context only.
...
user_input = st.text_input("Input your prompt here", key=prompt_key)
...
if user_input:
    ...
    # Clear the input box by generating a new unique key
    st.experimental_rerun()

# Display ongoing conversation with custom styling
for speaker, utterance in st.session_state.conversation[::-1]:
    if speaker == "User":
